...
| `NavigationConfig` | `Domain/NavigationConfig.cs` | SO: `EmptyCost=1`, `TerritoryCost=8`, `LineCost=1`, `MaxSeedOffset=64`, `RebuildIntervalSeconds=0`, `AllowDiagonal=true`. Меню: `Flood/Navigation/Navigation Config`. |
...
| `FlowFieldNavigator` | `Domain/FlowFieldNavigator.cs` | MonoBehaviour. Подписан на `ArenaState.Floor.Painted/PaintedSilent` и `ArenaState.TerritoryErased` → dirty. `Update`: rebuild при dirty с дросселем `RebuildIntervalSeconds`. API: `SampleDirection(worldXZ)`, `Field`, `IsDirty`, `MarkDirty()`. |
...
- **Dirty-цикл.** `MarkDirty()` → ближайший `Update()` пересобирает (если прошёл `RebuildIntervalSeconds`). События `Painted/PaintedSilent/TerritoryErased` ставят dirty автоматически. Один rebuild за кадр максимум.
...
- **Обструкция поверх грида.** Если появятся неклеточные препятствия (турели, трупы громил), добавить дополнительный bool-слой в `FlowFieldBuilder.Build` — клетка → infinite cost.
